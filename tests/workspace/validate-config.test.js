...
describe("Shared Configuration Inheritance", () => {
  ...
  const utilsPackageJsonPath = path.join(
    repoRoot,
  ...
  test(".config directory exists with shared configs", () => {
    ...
    expect(fs.existsSync(path.join(configDir, "prettier.config.js"))).toBe(
      true,
    );
    expect(fs.existsSync(path.join(configDir, "jest.config.js"))).toBe(true);
    expect(fs.existsSync(path.join(configDir, "tsconfig.base.json"))).toBe(
      true,
    );
  });
  ...
  test("packages/ui extends shared ESLint config", () => {
    ...
    const uiPackageJson = JSON.parse(
      fs.readFileSync(uiPackageJsonPath, "utf8"),
    );

    // Check for eslintConfig or extends in package.json
    const hasEslintConfig =
      uiPackageJson.eslintConfig ||
  ...
  test("packages/utils extends shared ESLint config", () => {
    ...
    const utilsPackageJson = JSON.parse(
      fs.readFileSync(utilsPackageJsonPath, "utf8"),
    );

    const hasEslintConfig =
      utilsPackageJson.eslintConfig ||
